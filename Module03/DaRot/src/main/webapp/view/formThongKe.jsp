...
">
    ...
    <div class="col-lg-12">
        ...
        <h1 style="margin-top: 5px; font-family: 'Baskerville Old Face'; text-align: center; font-size: 25px">Thong Ke Sach</h1>
                <div class="row col-lg-12" style=" display: flex;align-items: center; justify-content: center;" >
                    <form method="post" action="/muon?action=search">
<%--                        <button type="submit" formmethod="get"  name="action" value="add" class="d-inline ml-5 btn btn-secondary">Add</button>--%>

                        <input name="searchTacGia"  type="text"  class="d-inline" placeholder="Tac Gia" value="${sTacGia}">
...
<%--                        </c:choose>--%>
                        ...
                        <select name="searchHocSinh" style="width: 300px" class="form-control d-inline">
                            <option value="" > All Hoc Sinh </option>
                            <c:forEach items="${hocSinhList}" var="shocSinh"  varStatus="i">
                                <c:if test="${ shocSinh.getTen().toString().trim()  ==  sHocSinh.trim()}">
                                    <option selected value="${shocSinh.getMaHocSinh()}"> ${shocSinh.getTen()} </option>
                                </c:if>

                                <c:if test="${ shocSinh.getTen().toString().trim()  !=  sHocSinh.trim()}">
                                    <option          value="${shocSinh.getMaHocSinh()}"> ${shocSinh.getTen()} </option>
                                </c:if>
                            </c:forEach>
                        </select>

                        <select name="searchSach" style="width: 300px" class="form-control d-inline">
                            <option value="" > All Sach </option>
                            <c:forEach items="${sachList}" var="searchSach"  varStatus="i">
                                <c:if test="${ searchSach.getTenSach().toString()  ==  sSach}">
                                    <option selected value="${ searchSach.getMaSach()}"> ${searchSach.getTenSach()} </option>
                                </c:if>

                                <c:if test="${ searchSach.getTenSach().toString()  !=  sSach}">
                                    <option          value="${searchSach.getMaSach()}"> ${searchSach.getTenSach()} </option>
                                </c:if>
                            </c:forEach>
        ...
        <div class="row col-lg-12 col-md-12 col-12 "  style="display: block;overflow: auto;margin-right: 0px;margin-left: 0px;padding-left: 0px;padding-right: 0px;margin-top: 0px;">
            <table id="tablePhanTrang" width="100%"  class="  table table-striped table-bordered  "   >
                ...
                <tbody>
                <c:if test="${theMuonList != null}" >
                    <c:forEach items="${theMuonList}" var="the" varStatus="i">
                        <tr>

                            <td > ${   the.getMaMuon()}    </td>
                            <td > ${   the.getHocSinh().getTen() }    </td>
                            <td > ${   the.getSach().getTenSach()}    </td>
                            <td > ${   the.getSach().getTacGia()}    </td>
                            <td > ${   the.getNgayMuon()}    </td>
                            <td > ${   the.getNgayTra()}    </td>

                            <c:if test="${  the.isStatus()  == true}"> <td > Đang MƯợn   </td> </c:if>
                            <c:if test="${  the.isStatus() == false  }"> <td > Đã Trả   </td> </c:if>
                            <td>
                                <a href="/muon?action=tra&id=${  the.getMaMuon() }"><button    class="btn btn-outline-info" >
                                    Trả
                                </button></a>
                            </td>
                                <%--                            <td>--%>
